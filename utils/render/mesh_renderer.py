...
from .camera_utils import look_at, perspective
...
def phong_shader(normals,
    ...
    batch_size, image_height, image_width = [
        s.value for s in normals.shape[:-1]]
    ...
    if ambient_color is not None:
        ...
        output_colors = tf.add(
            output_colors, ambient_reshaped * diffuse_colors)
    ...
    if camera_position is not None:
        ...
        reflection_direction_dot_camera_direction = tf.clip_by_value(
            tf.nn.l2_normalize(
                reflection_direction_dot_camera_direction, dim=2),
            0.0, 1.0)
    ...
    alpha_images = tf.reshape(
        alphas, [batch_size, image_height, image_width, 1])
...
def mesh_renderer(vertices,
    ...
    Args:
      ...
      detla_center: float, 0D tensor, or 1D tensor with shape [batch_size] specifying
          desired projective center.
    ...
    if specular_colors is not None:
        ...
        vertex_attributes = tf.concat(
            [normals, vertices, diffuse_colors], axis=2)
    ...
    pixel_attributes = rasterize_triangles(
        ...
        image_width, image_height, [-1] * vertex_attributes.shape[2].value, delta_center=delta_center)
    ...
    if specular_colors is not None:
        ...
        if len(shininess_coefficients.shape) == 2:
            ...
            shininess_coefficients = tf.reshape(
                shininess_coefficients, [-1, 1, 1])

    pixel_mask = tf.cast(tf.reduce_any(
        diffuse_colors >= 0, axis=3), tf.float32)
